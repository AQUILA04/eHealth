...
public class PharmacyWorkflowService {
    ...
    @Transactional public ProductResponse createProduct(CreateProductRequest request) {
        productRepository.findBySku(request.sku()).ifPresent(p -> { throw new IllegalArgumentException("SKU déjà utilisé: " + request.sku()); });
    ...
    public List<ProductResponse> listProducts() { return productRepository.findAll().stream().map(this::toProductResponse).toList(); }
    @Transactional public LotResponse receiveLot(ReceiveLotRequest request) {
        var product = productRepository.findById(request.productId()).orElseThrow(() -> new EntityNotFoundException("Produit introuvable: " + request.productId()));
    ...
    public List<LotResponse> listLots(Long productId) { var lots = productId == null ? lotRepository.findAll() : lotRepository.findByProductIdOrderByExpiryDateAsc(productId); return lots.stream().map(this::toLotResponse).toList(); }
    @Transactional public DispensationResponse validate(CreateDispensationRequest request) {
        var product = productRepository.findById(request.productId()).orElseThrow(() -> new EntityNotFoundException("Produit introuvable: " + request.productId()));
        var available = lotRepository.findByProductIdAndExpiryDateGreaterThanOrderByExpiryDateAsc(product.getId(), LocalDate.now()).stream().mapToInt(InventoryLot::getQuantityOnHand).sum();
    ...
    @Transactional public DispensationResponse dispense(Long id, DispenseRequest request) {
        var dispensation = dispensationRepository.findById(id).orElseThrow(() -> new EntityNotFoundException("Dispensation introuvable: " + id));
        ...
        InventoryLot lot = request.lotId() == null ? lotRepository.findByProductIdAndExpiryDateGreaterThanOrderByExpiryDateAsc(dispensation.getProduct().getId(), LocalDate.now()).stream().filter(l -> l.getQuantityOnHand() >= dispensation.getQuantity()).findFirst().orElseThrow(() -> new IllegalStateException("Aucun lot FEFO ne couvre la quantité demandée.")) : lotRepository.findById(request.lotId()).orElseThrow(() -> new EntityNotFoundException("Lot introuvable: " + request.lotId()));
    ...
    public List<DispensationResponse> listDispensations(String patientRef) { var rows = patientRef == null || patientRef.isBlank() ? dispensationRepository.findAll() : dispensationRepository.findByPatientRefOrderByValidatedAtDesc(patientRef); return rows.stream().map(this::toDispensationResponse).toList(); }
    private ProductResponse toProductResponse(MedicationProduct p) { int quantity = lotRepository.findByProductIdOrderByExpiryDateAsc(p.getId()).stream().filter(l -> !l.getExpiryDate().isBefore(LocalDate.now())).mapToInt(InventoryLot::getQuantityOnHand).sum(); return new ProductResponse(p.getId(), p.getSku(), p.getName(), p.getGenericName(), p.getAtcCode(), p.getUnit(), p.getMinimumStock(), quantity, quantity <= p.getMinimumStock(), p.isActive()); }
